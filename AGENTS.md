...
- **Plugin page-route contribution** (added 2026-05-11, #623) — `definePlugin({ pageRoutes: NpPluginPageRouteRegistration[] })`. Each entry has `pattern`, `component`, optional `metadata`, plus `surface: "site" | "member"` and `locale: "auto" | "none"` knobs. The catch-all dispatches via `dispatchPluginRoute` (`@nexpress/next`); precedence is page > slug-redirect > theme > plugin > 404. Adding optional fields to the registration type is non-breaking. Author docs: `docs/plugin-pages.md`. Pattern grammar (`/`, `:name`, `:name(regex)`, segment-count match) is stable; glob / catch-all is **not** part of the v0.1 commitment. The `surface: "member"` shell wrap is **experimental** until PRT.4 lands the parallel `(member)` catch-all.
...
- **Plugin route `surface: "member"` shell wrap** — accepted on the registration type today, but member-surface plugin routes still render in the `(site)` shell. The wrap requires a parallel `(member)` catch-all in the host app — deferred to v0.2 (PRT.4 of the plugin-route track). Plugins that set `surface: "member"` should treat this as a hint, not a security boundary; gate access in the route component itself (`getSiteMember()` → `notFound()` on null).
