...
describe.skipIf(skipIfNoTestDb())("Runtime provider outage isolation", () => {
  ...
  it("keeps normal collection reads, writes and an unrelated maintenance handler available after provider failure", async () => {
    ...
    try {
      ...
      await withCurrentSite(siteId, async () => {
        ...
        await saveDocument("posts", id, { title: "Updated during outage" }, f.actor.actor.user);
