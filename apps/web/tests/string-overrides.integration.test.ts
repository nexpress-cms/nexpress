 ...
 */
describe.skipIf(skipIfNoTestDb())("UI string overrides (Phase D)", () => {
  beforeAll(async () => {
  ...
  afterEach(async () => {
    const { resetStringOverrideCache, resetCurrentSiteResolver, resetStrings } =
      await import("@nexpress/core");
    resetStrings();
    resetStringOverrideCache();
    resetCurrentSiteResolver();
  ...
  it("t() returns the override when an admin sets one", async () => {
    const { addStrings, setStringOverride, t, withCurrentSite } = await import("@nexpress/core");
    addStrings("en", { greeting: "Hello" });
  ...
  it("overrides are per-site (default vs. another tenant)", async () => {
    const { addStrings, createSite, setStringOverride, t, withCurrentSite } =
      await import("@nexpress/core");
  ...
  it("deleteStringOverride removes the row entirely", async () => {
    const { deleteStringOverride, listStringOverridesForSite, setStringOverride, withCurrentSite } =
      await import("@nexpress/core");
  ...
  it("GET /api/admin/i18n/strings returns the merged registry + overrides (editor+)", async () => {
    ...
    const { addStrings, setStringOverride, withCurrentSite } = await import("@nexpress/core");
  ...
  it("PUT /api/admin/i18n/strings sets an override (admin-only)", async () => {
    ...
    const { addStrings, listStringOverridesForSite } = await import("@nexpress/core");
  ...
  it("DELETE drops the override row", async () => {
    ...
    const req = buildRequest("/api/admin/i18n/strings?locale=en&key=doomed", {
      session: admin,
      method: "DELETE",
    });
