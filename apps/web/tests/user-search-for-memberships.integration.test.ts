...
describe.skipIf(skipIfNoTestDb())(
  "user search for memberships picker (Phase 15.8)",
  () => {
    beforeAll(async () => {
    ...
    it("/api/users?search=email returns matching users (editor+)", async () => {
      ...
      expect(body.docs?.some((d) => d.email === "alice.target@example.com")).toBe(
        true,
      );
      expect(body.docs?.some((d) => d.email === "bob.other@example.com")).toBe(
        false,
      );
    });
