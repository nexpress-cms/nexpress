...
| `/policies`, `/policies/new`, `/policies/[id]` (3)                               | `useRuntimeResource`, Policy views, `AgentPolicySimulation`, shared frame.                                                                 | `agent-runtime.spec.ts`: typed Agent override draft and unchanged retry, bounded non-authorizing simulation, stale and inaccessible simulation evidence removal. `agent-states.spec.ts` now directly covers delayed loading, populated/empty results, retained refresh, 503, contract rejection, 403, stopped retry loops and keyboard recovery, with six viewport/theme captures. This closes the earlier policy-list fixture gap; it is not a visual pass for every error state. |
| `/budgets` (1)                                                                   | `AgentBudgetView`, independent Runtime-status and budget reads through `useRuntimeResource`, shared frame.                                 | `agent-runtime.spec.ts`: configured budget/operations facts, unknown measurements preserved as unknown. The extended journey directly covers failure/recovery of either independent read, receipt isolation, malformed Runtime timestamps, retained budget edits, 429 retry identity and authentication-loss clearing. The inspected visual evidence is the 320px observation view, not every budget-editor variant.                                                               |
