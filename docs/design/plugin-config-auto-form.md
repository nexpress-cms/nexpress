...
> Current-state note (2026-07-22): plugin configuration remains site-scoped in
> `np_settings`; activation has since moved from `np_plugins.enabled` to sparse
> `np_site_plugins` overrides. Older activation examples below are historical,
> not the live persistence contract.
> Prerequisites:
>
...
| E   | Settings persistence layer                                                 | **Drop `np_plugins.config`, store under `np_settings` with key `plugin.config:<id>`** | Repo is pre-1.0 / private — migrating now (data copy + column drop in one migration) trades ~150 LOC for permanent symmetry with theme settings (`theme.settings:<id>`), shared internal helpers (e.g. `getCachedSetting<T>(key)`), and matching function signatures (`getThemeSettings` ↔ `getPluginConfig`). After v1.0 this asymmetry would be locked in. The later site-activation contract keeps `np_plugins` as installation inventory and stores activation separately. |
...
versioned envelope), then drops the column. Post-G.1, `np_plugins` was a lean
state row. The current contract further separates process-wide installation
(`np_plugins`) from site activation (`np_site_plugins`).
