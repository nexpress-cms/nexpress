...
| `member-profile-activity.integration.test.ts` (3)       | explicit collection opt-in, public/site visibility, stable exact document pagination, comment target anchors, and matching PII-free profile/activity HTTP wires                                     |
