...
| `provides.{blocks, patterns, fields, hooks, actions, apiRoutes, pageRoutes, scheduledTasks, adminExtensions, collections}` | derived from declared surface | Catalog metadata. Pattern IDs end up in `provides.patterns`, block types in `provides.blocks`, route patterns in `provides.pageRoutes`, etc. Author-declared entries merge with derived ones. |
