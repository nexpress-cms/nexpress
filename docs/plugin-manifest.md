...
| `provides.{blocks, patterns, templates, translations, hooks, actions, apiRoutes, pageRoutes, scheduledTasks, adminExtensions, collections}` | derived from declared surface | Catalog metadata. Template and translation keys are derived alongside blocks, patterns, routes, tasks, and actions. Author-declared entries merge with derived ones. |
...
definePlugin<MyPluginConfig>({
  ...
  pageRoutes: [ /* ... */ ],
  blocks: [ /* ... */ ],
  templates: { /* collection: { id: { label, component } } */ },
  i18n: { /* locale: { key: ICU message } */ },
...
| `routes`         | Typed plugin API routes mounted under `/api/plugins/<id>`. Mutations may opt into bounded exact request bytes with `bodyMode: "raw"`; any route may opt into bounded byte responses with `responseMode: "binary"`. See [`plugin-api-routes.md`](plugin-api-routes.md). | When the plugin needs a static namespaced HTTP surface, signed callback intake, or authenticated file delivery.                                  |
...
| `setup`          | `(ctx) => void` invoked once per plugin load and validated at runtime.                                                                                                                                                                                                 | Validate environment, log a startup line, or use the compatible `ctx.actions.register*` API for genuinely dynamic/legacy actions.                |
| `teardown`       | `() => void` called before reload/replacement in reverse load order.                                                                                                                                                                                                   | Release timers, sockets, or other resources created during setup.                                                                                |
| `i18n`           | Validated BCP 47 + ICU locale bundles. See [`plugin-i18n.md`](plugin-i18n.md).                                                                                                                                                                                         | When the plugin renders user-facing copy.                                                                                                        |
| `templates`      | Validated page-template contributions per collection. See [`plugin-templates.md`](plugin-templates.md).                                                                                                                                                                | Plugins that ship templates for the document dispatcher.                                                                                         |
| `patterns`       | Validated page-builder pattern presets. See [`plugin-patterns.md`](plugin-patterns.md).                                                                                                                                                                                | When the plugin ships pre-shaped block trees; bootstrap assigns their concrete source and verifies every referenced block type.                  |
...
- [`plugin-patterns.md`](plugin-patterns.md) — recursive pattern definitions,
  source assignment, block references, collisions, and diagnostics.
- [`plugin-templates.md`](plugin-templates.md) — page-template metadata,
  components, precedence, reload, and diagnostics.
- [`plugin-i18n.md`](plugin-i18n.md) — locale tags, ICU messages, layering,
