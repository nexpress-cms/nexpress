...
| `routes`         | Typed plugin API routes mounted under `/api/plugins/<id>`. Mutations default to parsed JSON and may opt into bounded exact bytes with `bodyMode: "raw"`. See [`plugin-api-routes.md`](plugin-api-routes.md). | When the plugin needs a static, namespaced HTTP surface or signed callback intake.                                                               |
