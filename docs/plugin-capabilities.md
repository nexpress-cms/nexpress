...
`definePlugin` auto-derives two families from the declared surface so
you don't have to repeat yourself:

- **Routes** — any entry in `routes: [...]` adds `api:route`.
...
or `ctx.http.fetch`, so silently granting those would be a privilege
escalation bug.
...
| `site:route` | reserved | Future site-side route contributions. |
| `admin:panel` | `admin: {...}` extensions | Documentation; not enforced at runtime today. |
| `admin:dashboard` | `admin.dashboardWidgets` | |
| `admin:collection-tab` | `admin.collectionTabs` | |
| `hooks:content` | `content:before/afterCreate`, `content:before/afterUpdate`, `content:before/afterDelete`, `content:before/afterPublish` | Auto-derived. |
| `hooks:auth` | `auth:afterLogin`, `auth:beforeLogout`, `auth:afterRegister` | Auto-derived. |
| `hooks:render` | `render:beforePage`, `render:afterPage` | Auto-derived. See [`plugin-render.md`](plugin-render.md). |
| `hooks:scheduled` | `scheduled:*` (reserved) | Auto-derived; `scheduled` task entries don't need it today. |
