...
| `impl.members.publicProfile`                                | Renders `/u/{handle}` from prepared `profile`, exact `activity`, route-owned links/labels, and an opaque follow action. It must not query member or collection state. Omit it to use the complete framework `.np-member-profile*` fallback; the community theme is the live full renderer.                                                                                     |
...
| `--np-member-oauth-{google,github}-*`                       | CSS custom properties for the member-login OAuth buttons (`.np-members-auth` scope). When OAuth providers are registered, `/members/login` renders "Continue with ..." links that start `/api/members/oauth/{provider}/start` (M.2).                                                                                                                                           |
...
4. **`src/styles.ts`** (or your theme CSS) — override `--np-member-form-*` tokens scoped under `.<your-theme-root> .np-members-form`, and OAuth button tokens under `.<your-theme-root> .np-members-auth`. Discussion / comment forms keep their global look; member auth surfaces pick up your theme's edge.
5. **`src/index.ts`** — declare `impl.members.{shell, notFound}` and, when the theme owns the public profile presentation, `publicProfile`.
