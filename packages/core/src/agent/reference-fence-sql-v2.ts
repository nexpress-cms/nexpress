 ...
 */
export const NP_AGENT_REFERENCE_FENCE_SQL_V2 = `
...
BEGIN
  ...
  bound_owner_kinds := CASE bound_owner_kind
    WHEN 'runtime-audit' THEN ARRAY['runtime-audit','studio-audit']
    WHEN 'read-invocation' THEN ARRAY['read-invocation','admin-invocation']
  ...
  IF TG_OP='UPDATE' AND bound_owner AND next_body IS DISTINCT FROM previous_body THEN
    ...
    END IF;
    IF NOT audit_metadata_update AND NOT invocation_metadata_update AND NOT action_detachment THEN
      RAISE EXCEPTION USING ERRCODE='23514', MESSAGE='Agent source evidence is immutable.';
    END IF;
  END IF;
