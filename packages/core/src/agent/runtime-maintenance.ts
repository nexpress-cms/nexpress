...
export async function collectAgentRuntimeMaintenanceV1(options: { siteId: string; now?: Date }) {
  const args = input(options, ["siteId", "now"]);
  const now = clock(args.now);
  return npWithAgentRuntimeControlTransactionV1(options.siteId, async ({ db, settings }) => {
