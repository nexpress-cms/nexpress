...
async function saveDocumentImpl(
  ...
  // member edit it (zod-validated `data` doesn't include it; we
  // always overwrite here to defeat any sneak-through). Member
  // updates must NOT change the column — so we only set it on
  // create.
  if (actor.kind === "member" && operation === "create") {
