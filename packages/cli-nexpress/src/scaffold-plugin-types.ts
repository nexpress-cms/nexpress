 ...
 *
 * Responses serialize as JSON by default. \`responseMode: "binary"\` instead
 * requires non-empty \`Uint8Array\` bytes (bounded to 8 MiB) plus a
 * \`Content-Type\` header. The download route below keeps staff auth enabled
 * and marks the response private/no-store.
 *
...
export const ${names.exportName} = definePlugin({
  ...
  routes: [
    ...
    },
    {
      ...
      auth: true,
      responseMode: "binary",
      description: "Download one bounded, non-cached text artifact.",
      handler: () => ({
        status: 200,
        body: new TextEncoder().encode("Generated by ${names.pluginId}\\n"),
        headers: {
          "Cache-Control": "private, no-store",
          "Content-Disposition": 'attachment; filename="${names.pluginId}.txt"',
...
bounded exact-byte contract required by signed callbacks.
The staff-only \`GET /api/plugins/${names.pluginId}/download\` route demonstrates
bounded binary response delivery without base64 or a public storage URL. Edit
