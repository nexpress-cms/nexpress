...
import type { NpBootstrapIntent, NpDb, NpReloadPluginsResult } from "@nexpress/next";
...
export const reloadPlugins: () => Promise<NpReloadPluginsResult> = async () => ({
  reloaded: true,
  schedules: null,
});
export const ensureFor: (intent: NpBootstrapIntent) => Promise<void> = async () => {};
export const shutdownBootstrap: () => Promise<void> = async () => {};
