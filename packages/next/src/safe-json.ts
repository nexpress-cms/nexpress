 ...
 * `NxValidationError` (which the response mapper turns into a 400 with
 * a friendly `{ error: { code: "VALIDATION_ERROR", … } }`).
 *
 * Routes used to call `await request.json()` directly inside the
 * outer `try` and rely on `nxErrorResponse(error)` for the catch.
 * That worked for `NxError` subclasses and zod issues, but plain
