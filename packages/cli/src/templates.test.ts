...
describe("getProjectFiles", () => {
  ...
  it("scaffolds opt-in Toss payment guidance without enabling credentials", () => {
    ...
    expect(env).toContain("createShop({ packaging: { adapter } })");
    expect(env).toContain("createShop({ packing: { adapter } })");
    expect(env).toContain("createShop({ carrier: { adapter } })");
    expect(env).toContain("proposeParcels is a read-only calculation");
    expect(env).toContain("same-item replacement line, product, slug, SKU identifiers");
    expect(env).toContain("exact PII-free parcel result within its 60-second lifetime");
    expect(env).toContain("never mutate provider state");
    expect(env).toContain("provider I/O outside transactions");
    expect(env).toContain("compare-and-swaps the source and parcel revisions");
    expect(env).toContain("manual parcel JSON remains the fallback");
    expect(env).toContain("receive no address or delivery data");
    expect(env).toContain("physical packing/WMS");
    expect(env).toContain("Implement createPackingWork and cancelPackingWork together");
    expect(env).toContain("exactly one durable packing work per target/order");
    expect(env).toContain("stable create/cancel UUIDs");
    expect(env).toContain("does not impose an adapter-call timeout");
    expect(env).toContain("Apply a finite bound to every");
    expect(env).toContain("timeout/ambiguity stays retryable");
    expect(env).toContain("Active work blocks parcel edits");
    expect(env).toContain("local shipment completion consumes it");
    expect(env).toContain("confirm packing cancellation before compensation");
    expect(env).toContain("permanent workId+cancellationId provider tombstone");
    expect(env).toContain("dominates delayed/retried creation");
    expect(env).toContain("retains one cancelled target/order");
    expect(env).toContain("rejects recreation");
    expect(env).toContain("Only an unattached");
    expect(env).toContain(
      "cancelled tombstone reopens manual parcel/carrier/compensation fallback",
    );
    expect(env).toContain("Before verified tracking");
    expect(env).toContain("cancellation of its exact carrier shipment");
    expect(env).toContain("After verified tracking, carrier");
    expect(env).toContain("cancellation and automatic restock fail closed");
    expect(env).toContain("only exact booked shipment");
    expect(env).toContain("WMS cancellation started before tracking");
    expect(env).toContain("with the same cancellation UUID");
    expect(env).toContain("Stored provider-confirmed and cancel-confirmed transitions");
    expect(env).toContain("finalize locally without it");
    expect(env).toContain("unresolved provider create/cancel/reconcile I/O");
    expect(env).toContain("needs the original adapter");
    expect(env).toContain("Non-terminal work preserves its commercial source order");
    expect(env).toContain("Finalize exact confirmed state locally");
    expect(env).toContain("reconciliation under its stable UUID");
    expect(env).toContain("manual ship path");
    expect(env).toContain("No generic override terminalizes pending/manual-review");
    ...
    expect(readme).toContain("Optional packaging proposals");
    expect(readme).toContain("read-only parcel proposals");
    expect(readme).toContain("outbound and same-item replacement identifiers");
    expect(readme).toContain("valid for at most 60 seconds");
    expect(readme).toContain("compare-and-swaps the current fulfillment or exchange");
    expect(readme).toContain("manual parcel JSON");
    expect(readme).toContain("remains independent from carrier");
    expect(readme).toContain("Optional packing work");
    expect(readme).toContain("pairs `createPackingWork` and `cancelPackingWork`");
    expect(readme).toContain("exactly one durable work per target/order");
    expect(readme).toContain("exact PII-free outbound or same-item");
    expect(readme).toContain("replacement lines and parcels");
    expect(readme).toContain("same revision/fingerprint");
    expect(readme).toContain("does not impose an adapter-call timeout");
    expect(readme).toContain("every adapter network operation needs a finite bound");
    expect(readme).toContain("transport ambiguity remains retryable");
    expect(readme).toContain("Direct full refund or replacement cancellation");
    expect(readme).toContain("permanent `workId + cancellationId` tombstone");
    expect(readme).toContain("cancellation dominates delayed or retried creation");
    expect(readme).toContain("single `cancelled` target/order tombstone");
    expect(readme).toContain("until order cleanup and rejects recreation");
    expect(readme).toContain("rejects recreation");
    expect(readme).toContain("Only an unattached `cancelled`");
    expect(readme).toContain("tombstone reopens manual parcel, carrier, refund");
    expect(readme).toContain("Before verified tracking");
    expect(readme).toContain("cancelling its exact carrier shipment");
    expect(readme).toContain("After verified tracking");
    expect(readme).toContain("automatic inventory restock fail closed");
    expect(readme).toContain("only exact booked");
    expect(readme).toContain("packing conflict remains diagnosed");
    expect(readme).toContain("WMS cancellation started before tracking");
    expect(readme).toContain("same cancellation UUID");
    expect(readme).toContain("diagnostics remain visible after adapter removal");
    expect(readme).toContain("non-terminal work preserves its commercial source order");
    expect(readme).toContain("member-linked owner segment");
    expect(readme).toContain(
      "`provider-confirmed` and `cancel-confirmed` transitions finish locally",
    );
    ...
    expect(readme).toContain("existing manual ship path");
    expect(readme).toContain("no generic override terminalizes `pending` or");
    expect(readme).toContain("`manual-review`");
    expect(readme).toContain("or delete the site");
    expect(readme).toContain("Private retention is unchanged");
    expect(readme).toContain("Omitting the adapter preserves the existing manual flows");
    ...
    expect(readme).toContain("pair `bookExchangeShipment` with `cancelExchangeShipment`");
