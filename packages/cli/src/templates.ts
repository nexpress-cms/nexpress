...
function nexpressConfigTemplate(config: TemplateConfig): string {
  ...
  // file. `pnpm setup` writes the right env block directly.
...
function envExampleTemplate(_config: TemplateConfig): string {
  // `pnpm setup` is the recommended path — this template stays
  ...
  return [
    "# Generated by create-nexpress. Run `pnpm setup` for an interactive wizard.",
...
pnpm install
${dockerStep}
pnpm setup              # browser env wizard (DB / NX_SECRET / storage / migrations)
pnpm dev
\`\`\`
