...
function envExampleTemplate(config: TemplateConfig): string {
  ...
  return [
    ...
    "",
    "# Community realtime SSE capacity is process-local. Values are canonical",
    "# integers in 1..10000 and the per-site limit cannot exceed the process limit.",
    "# NP_COMMUNITY_REALTIME_MAX_STREAMS=200",
    "# NP_COMMUNITY_REALTIME_MAX_SITE_STREAMS=50",
    "",
    ...
    "",
    "# Optional Toss Payments v2 initiation, refunds, and provider-cancellation reconciliation for Shop.",
    ...
    "",
    "# Optional server-only Shop shipping quote adapter. Without one, Admin-managed",
    "# local shipping policies provide base rates, free thresholds, and surcharges;",
    "# no published policies preserves the zero-shipping fallback.",
    "# Replace the default Shop instance with one createShop({ shipping: { adapter } })",
    "# factory. The adapter overrides local policies. Destination PII must stay out",
    "# of logs and adapter results.",
    "",
    ...
    "",
    "# Optional independent server-only Shop packing-work adapter.",
    "# Replace the default Shop instance with one createShop({ packing: { adapter } })",
    "# factory. Implement createPackingWork and cancelPackingWork together.",
    "# V1 permits exactly one durable packing work per target/order.",
    "# Shop freezes exact PII-free outbound/replacement lines and parcels behind",
    "# stable create/cancel UUIDs, calls providers outside transactions, and",
    "# durably confirms provider effects before local completion or reconciliation.",
    "# Shop does not impose an adapter-call timeout. Apply a finite bound to every",
    "# network operation; timeout/ambiguity stays retryable with the same stable",
    "# pending or cancel-pending identity.",
    "# Active work blocks parcel edits. Only an exact parcel-aware carrier booking",
    "# may attach; local shipment completion consumes it. Direct full refund and",
    "# replacement cancellation confirm packing cancellation before compensation.",
    "# Keep a permanent workId+cancellationId provider tombstone so cancellation",
    "# dominates delayed/retried creation. Shop retains one cancelled target/order",
    "# tombstone until order cleanup and rejects recreation. Only an unattached",
    "# cancelled tombstone reopens manual parcel/carrier/compensation fallback.",
    "# Before verified tracking, an attached cancellation may unwind only through",
    "# cancellation of its exact carrier shipment. After verified tracking, carrier",
    "# cancellation and automatic restock fail closed; only exact booked shipment",
    "# completion may proceed, with the packing conflict diagnosed and retained.",
    "# A WMS cancellation started before tracking may retry/reconcile/finalize after",
    "# tracking with the same cancellation UUID. Diagnostics stay visible after",
    "# adapter removal. Stored provider-confirmed and cancel-confirmed transitions",
    "# finalize locally without it; unresolved provider create/cancel/reconcile I/O",
    "# needs the original adapter.",
    "# Non-terminal work preserves its commercial source order, including the member",
    "# link, past 365 days. Finalize exact confirmed state locally, complete provider",
    "# reconciliation under its stable UUID, consume exact active work by the existing",
    "# manual ship path, or delete the site.",
    "# No generic override terminalizes pending/manual-review.",
    "# Private retention is unchanged.",
    "# Optional verifyPackingStatusWebhook and lease-safe readPackingStatus polling",
    "# reconcile monotonic PII-free accepted/picking/failed/packed evidence.",
    "# Packed evidence never ships or consumes work. This capability does not add",
    "# picking/bin/worker state, addresses, rates, labels, material inventory,",
    "# authoritative completion policy or provider-specific WMS protocols.",
    "# Omission preserves manual flows.",
    "",
    ...
    "# An optional bookShipmentWithParcels consumes a revision-safe PII-free",
    "# mm/gram package snapshot that Admin locks to the durable shipment UUID.",
    "# Outbound or replacement parcel-aware adapters may add schedulePickup",
    "# and cancelPickup together; Shop keeps shipment-keyed pickup state.",
    "# Then pass one server-only opaque pickupLocationReference beside adapter;",
    ...
    "# over that complete pickup pair. Return 1-20 ordered non-overlapping UTC",
    "# windows valid for at most one hour. Shop stores only a short-lived PII-free",
    "# snapshot and passes the selected bounds to unchanged schedulePickup v1.",
    "# General calendars, recurrence, charges, addresses, and automatic scheduling",
    "# remain outside this additive provider capability.",
    "# Optional acquireShippingLabel requires readShippingLabel and uses one stable",
    "# acquisition UUID for first purchase or atomic regeneration. Shop persists",
    "# only the PII-free label reference; bytes and URLs remain transient.",
    "# Verified tracking blocks purchase/regeneration. Provider charges, paper",
    "# Optional voidShippingLabel binds a stable void UUID to the exact current",
    "# generation. It blocks new intent after tracking and keeps booking intact.",
    "# Provider charges/refunds, paper layout, and protocols stay outside Shop.",
    ...
    "# Optional return-postage quotes require quoteReturnShipping and",
    "# createQuotedReturnShipment together. Shop freezes one exact method",
    "# while its separate origin sidecar lives for at most one hour; the adapter",
    "# must not charge, automatically deduct a refund, or decide who pays.",
    "# A payment adapter may independently add refundReturnSettlement. After",
    "# physical receipt, direct staff designate merchant or customer responsibility;",
    "# Shop either absorbs the quote or deducts that exact same-currency amount",
    "# from one positive net refund. Never create a separate/off-session charge.",
    "# Optional verifyReturnTrackingWebhook and readReturnTracking capabilities",
    "# advance a separate PII-free reverse-shipment state. Carrier delivery never",
    "# receives the physical return, restores inventory, or issues a refund.",
    "# After direct staff receive a return, Shop can create one exact same-item",
    "# replacement with reservation-aware inventory and manual carrier/tracking.",
    "# The owner submits one new replacement address under a 15-minute authority;",
    "# keep that separate private sidecar for at most 24 hours, audit direct-staff",
    "# access, require access before processing, then delete it on processing.",
    "# A carrier may add bookExchangeShipment and cancelExchangeShipment together.",
    "# Shop persists stable booking/cancellation UUIDs, calls providers outside",
    "# transactions, and deletes the replacement address only after durable",
    "# provider confirmation. Pending/confirmed work is explicitly resumable;",
    "# never echo the address in a result or treat ambiguous cancellation as final.",
    "# Optional bookExchangeShipmentWithParcels adds one exact v2 request.",
    "# Direct staff first save a PII-free mm/gram allocation over every immutable",
    "# replacement line; Shop locks it to the shipment UUID before provider I/O.",
    "# Retries must preserve that UUID and parcel revision; v1 remains valid when",
    "# the additive parcel-aware method is absent.",
    "# The independent readShippingLabel capability serves completed outbound",
    "# and replacement bookings through transient direct-staff PDF/PNG/ZPL bytes;",
    "# never persist or log those bytes or a provider label URL.",
    "# Optional acquireShippingLabel adds durable purchase/regeneration for both",
    "# shipment kinds. Treat acquisitionId as the provider idempotency key and",
    "# atomically replace replacesLabelReference for regeneration; never return bytes.",
    "# Optional voidShippingLabel uses voidId for the exact current generation.",
    "# Keep the booking intact; verified tracking blocks only a new void intent.",
    "# Existing schedulePickup and cancelPickup methods reuse the exact locked",
    "# replacement parcels without exchange or address data. Cancel the pickup",
    "# before provider shipment cancellation or automatic inventory restock.",
    "# Existing verifyTrackingWebhook and readTracking methods also serve exact",
    "# completed replacement bookings. Keep replacement tracking separate from",
    "# exchange status; verified state blocks provider cancellation/restock.",
    "# Never reuse the deleted outbound address, substitute another item,",
    "# collect a price difference, or automatically approve the exchange.",
    "",
