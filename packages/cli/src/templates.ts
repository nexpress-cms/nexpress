...
function envExampleTemplate(config: TemplateConfig): string {
  ...
  return [
    ...
    "# Parcel-aware adapters may add schedulePickup and cancelPickup together.",
...
5. A carrier may pair \`bookExchangeShipment\` with \`cancelExchangeShipment\`. Shop persists stable provider idempotency, deletes the replacement destination only after durable confirmation, and makes ambiguous booking or cancellation explicitly resumable. Its independent \`readShippingLabel\` capability may retrieve completed outbound and replacement labels as transient direct-staff bytes. Existing \`verifyTrackingWebhook\` and \`readTracking\` methods also serve exact completed replacement bookings through separate owner-visible state; any verified tracking state blocks provider cancellation and automatic restock. Label purchase/regeneration and replacement pickup remain separate.
