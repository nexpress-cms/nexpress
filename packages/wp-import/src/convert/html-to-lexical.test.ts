...
function blockTypes(out: ReturnType<typeof htmlToLexical>): string[] {
  return out.document.root.children.map((c) => c.type);
}

describe("htmlToLexical", () => {
  it("returns an empty paragraph for empty input", () => {
    const out = htmlToLexical("");
    expect(out.document.root.children).toHaveLength(1);
    expect(out.document.root.children[0]?.type).toBe("paragraph");
    expect(out.document.root.children[0]?.children).toEqual([]);
  });
  ...
  it("converts <h1>–<h6> to heading with the right tag", () => {
    ...
    expect(out.document.root.children[0]?.tag).toBe("h1");
    expect(out.document.root.children[1]?.tag).toBe("h3");
    expect(out.document.root.children[2]?.tag).toBe("h6");
  });
  ...
  it("applies bold/italic/code formats via bitmask", () => {
    ...
    const children = out.document.root.children[0]?.children ?? [];
  ...
  it("composes nested inline formats (bold + italic)", () => {
    ...
    const node = out.document.root.children[0]?.children?.[0];
  ...
  it("converts <ul>/<ol> to list blocks with the right listType", () => {
    ...
    expect(out.document.root.children[0]?.listType).toBe("bullet");
    expect(out.document.root.children[1]?.listType).toBe("number");
    expect(out.document.root.children[0]?.children).toHaveLength(2);
    expect(out.document.root.children[0]?.children?.[0]?.type).toBe("listitem");
  });
  ...
  it("converts <pre> to a code block holding the text content", () => {
    ...
    const block = out.document.root.children[0];
  ...
  it("converts <br> inside a paragraph to a linebreak inline node", () => {
    ...
    const types = out.document.root.children[0]?.children?.map((c) => c.type);
  ...
  describe("Phase 21.15 — Gutenberg block fences", () => {
    ...
    it("honors wp:heading {level} when the fence and markup disagree", () => {
      ...
      expect(out.document.root.children[0]?.type).toBe("heading");
      expect(out.document.root.children[0]?.tag).toBe("h4");
    });
    ...
    it("flips list type when the fence's `ordered` flag overrides the markup", () => {
      ...
      expect(out.document.root.children[0]?.type).toBe("list");
      expect(out.document.root.children[0]?.listType).toBe("number");
    });
    ...
    it("maps wp:separator (self-closing) to horizontalrule", () => {
      ...
      expect(out.document.root.children.map((c) => c.type)).toEqual([
        "paragraph",
        "horizontalrule",
        "paragraph",
      ]);
    ...
    it("maps wp:image figure markup to an image block plus caption paragraph", () => {
      ...
      expect(out.document.root.children.map((c) => c.type)).toEqual(["image", "paragraph"]);
      expect(out.document.root.children[0]).toMatchObject({
        type: "image",
        src: "https://example.com/hero.jpg",
        altText: "Hero",
      });
      expect(out.document.root.children[1]?.children?.[0]).toMatchObject({ text: "Hero caption" });
    });
    ...
    it("maps wp:embed iframe markup to a link paragraph and keeps the caption", () => {
      ...
      expect(out.document.root.children.map((c) => c.type)).toEqual(["paragraph", "paragraph"]);
      const link = out.document.root.children[0]?.children?.[0];
      expect(link).toMatchObject({
        type: "link",
        url: "https://www.youtube.com/embed/abc",
      });
      expect(out.document.root.children[1]?.children?.[0]).toMatchObject({ text: "Watch this" });
    });
    ...
    it("recurses through nested Gutenberg layout blocks so child attrs still apply", () => {
      ...
      expect(out.document.root.children.map((c) => c.type)).toEqual(["heading", "paragraph"]);
      expect(out.document.root.children[0]?.tag).toBe("h5");
    });
    ...
    it("falls through to the classic converter for unknown blocks", () => {
      ...
      expect(out.document.root.children[0]?.type).toBe("quote");
    ...
    it("warns on malformed Gutenberg attributes while preserving content", () => {
      ...
      expect(out.document.root.children[0]?.children?.[0]).toMatchObject({ text: "Safe" });
