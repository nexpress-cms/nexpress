...
- dd7ff70: Bumps `undici` from `^6.25.0` to `^8.3.0` (the bump deferred from #831's batch). Only direct surface we use is `new Agent({ connect })` for the DNS-pinned dispatcher in `media/download.ts`, which is unchanged across the 6 → 8 boundary.
